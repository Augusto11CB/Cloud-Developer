...
describe('concat', () => {
  ...
  it('should perform the concatenation of "Hello" and "World"', () => {
    const result = concat("Hello", "World");
    expect(result).to.equal("HelloWorld");
  });

  it('should throw an error if either of the strings are empty', () => {
    expect(() => {concat("Hello", "") }).to.throw('Null or Empty Strings are not allowed')
    expect(() => {concat("", "World") }).to.throw('Null or Empty Strings are not allowed')
    expect(() => {concat(null, null) }).to.throw('Null or Empty Strings are not allowed')
  });
});